...
class Statistics (ctk.CTkScrollableFrame):
    ...
    def _initCharts(self) -> None:
        ...
        sport_ratio_chart.grid(column = 1, row = 0,ipadx = 8, ipady = 8, padx = 9, pady = 9)
        time_chart = TimeColumnChart(self) # odtrénovaný čas za období
        time_chart.grid(column = 0, row = 1,ipadx = 5, ipady = 5, padx = 9, pady = 9)

        # testovací framy
        sport_chart = SportColumnChart(self) # graf poměru vykonaných sportů
